
# Halo 👋, saya `Fajrul Ramadhana Aqsa`

Saya seorang Web Developer (Backend), Data Analyst, dan praktisi Machine Learning & Deep Learning.

## Tentang Saya
- 🎓 Mahasiswa / Pengembang — Teknik Informatika
- 💼 Peran: Web Developer (Backend), Data Analyst
- 🤖 Keahlian ML/DL: machine learning, deep learning (implementasi model & evaluasi)
- 📫 Kontak: aqsafajrul@gmail.com • [LinkedIn](https://www.linkedin.com/in/fajrul-ramadhana-aqsa) • [GitHub](https://github.com/axadev-id)

## Keahlian
- Bahasa: Python, JavaScript/TypeScript, Java
- Backend: Node.js, Express, FastAPI
- Data & ML: pandas, scikit-learn, TensorFlow/PyTorch, NumPy
- Tools: Git, Docker, PostgreSQL, Linux, GitHub Actions

## Proyek Unggulan
1. **Spatial Rich Model SVM WOW Steganalysis Detector** — detektor steganalisis berbasis Spatial Rich Model dengan SVM dan metode WOW.
   - Repo: https://github.com/axadev-id/-Spatial-Rich-Model-SVM-WOW-Steganalysis-Detector
   - Peran: penelitian & implementasi model, evaluasi performa, preprocessing dataset
   - Teknologi: Python, scikit-learn, fitur SRM, SVM


## Statistik & Badges

  ![Anurag's GitHub stats](https://github-readme-stats.vercel.app/api?username=axadev-id&show_icons=true&theme=radical)


  ![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=axadev-id&layout=compact&theme=radical)


  ![Python](https://img.shields.io/badge/Python-3670A0?style=flat-square&logo=python&logoColor=white)
  ![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
  ![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat-square&logo=tensorflow&logoColor=white)

